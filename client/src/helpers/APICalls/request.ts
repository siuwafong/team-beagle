...
export async function getBookingsSitter(): Promise<Request> {
  ...
  return await fetch(`https://loving-sitter-team-beagle.herokuapp.com/request/bookings/sitter`, fetchOptions)
    .then((res) => res.json())
    .catch(() => ({
...
export async function getBookingsOwner(): Promise<Request> {
  ...
  return await fetch(`https://loving-sitter-team-beagle.herokuapp.com/request/bookings/owner`, fetchOptions)
    .then((res) => res.json())
    .catch(() => ({
      error: { message: 'Unable to connect to server. Please try again' },
    }));
